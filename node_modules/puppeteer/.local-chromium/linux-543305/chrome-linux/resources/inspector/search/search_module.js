Search.SearchView=class extends UI.VBox{constructor(){super(true);this.setMinimumSize(0,40);this.registerRequiredCSS('search/searchView.css');this._focusOnShow=false;this._isIndexing=false;this._searchId=1;this._searchMatchesCount=0;this._searchResultsCount=0;this._nonEmptySearchResultsCount=0;this._searchingView=null;this._notFoundView=null;this._searchConfig=null;this._pendingSearchConfig=null;this._searchResultsPane=null;this._progressIndicator=null;this._visiblePane=null;this.contentElement.classList.add('search-view');this._searchPanelElement=this.contentElement.createChild('div','search-drawer-header');this._searchPanelElement.addEventListener('keydown',this._onKeyDown.bind(this),false);this._searchPanelElement.addEventListener('input',this._onInput.bind(this),false);this._searchResultsElement=this.contentElement.createChild('div');this._searchResultsElement.className='search-results';const toolbar=new UI.Toolbar('search-toolbar',this._searchPanelElement);const searchContainer=this._searchPanelElement.createChild('div','search-container');this._search=UI.HistoryInput.create();searchContainer.appendChild(this._search);this._search.placeholder=Common.UIString('Search all sources (use "file:" to filter by path)\u200e');this._search.setAttribute('type','text');this._search.classList.add('search-config-search');this._search.setAttribute('results','0');this._search.setAttribute('size',42);const searchIcon=UI.Icon.create('mediumicon-search','search-icon');searchContainer.appendChild(searchIcon);this._searchInputClearElement=UI.Icon.create('mediumicon-gray-cross-hover','search-cancel-button');this._searchInputClearElement.classList.add('hidden');this._searchInputClearElement.addEventListener('click',this._onSearchInputClear.bind(this),false);const cancelButtonContainer=searchContainer.createChild('div','search-cancel-button-container');cancelButtonContainer.appendChild(this._searchInputClearElement);this._matchCaseButton=Search.SearchView._appendToolbarToggle(toolbar,'Aa',Common.UIString('Match Case'));this._regexButton=Search.SearchView._appendToolbarToggle(toolbar,'.*',Common.UIString('Use Regular Expression'));const searchStatusBarElement=this.contentElement.createChild('div','search-toolbar-summary');this._searchMessageElement=searchStatusBarElement.createChild('div','search-message');this._searchProgressPlaceholderElement=searchStatusBarElement.createChild('div','flex-centered');this._searchResultsMessageElement=searchStatusBarElement.createChild('div','search-message');this._scopesSelector=new UI.SegmentedButton();this._scopesSelector.element.classList.add('search-scopes');this._scopesSelector.show(this._searchPanelElement);this._advancedSearchConfig=Common.settings.createLocalSetting('advancedSearchConfig',new Search.SearchConfig(null,'',true,false).toPlainObject());this._searchScopes=new Map();this._defaultScope=Search.SearchView._readScopesExtenstions(this._scopesSelector,this._searchScopes);if(this._searchScopes.size<2)
this._scopesSelector.hideWidget();this._load();this._searchScope=null;}
...
static async openSearch(searchScopeType,query,searchImmediately){await UI.viewManager.showView('search.search');const searchView=(self.runtime.sharedInstance(Search.SearchView));searchView._toggle(searchScopeType,query,!!searchImmediately);return searchView;}
static _readScopesExtenstions(selector,scopesMap){let defaultScope=null;for(const extension of self.runtime.extensions(Search.SearchScope)){const id=extension.descriptor().id;scopesMap.set(id,extension);selector.addSegment(extension.title(),id,extension.descriptor().description);if(!defaultScope)
defaultScope=extension;}
return defaultScope;}
_buildSearchConfig(){return new Search.SearchConfig(this._scopesSelector.selected(),this._search.value,!this._matchCaseButton.toggled(),this._regexButton.toggled());}
async _toggle(searchScopeType,queryCandidate,searchImmediately){if(queryCandidate)
this._search.value=queryCandidate;this._selectScope(searchScopeType);if(this.isShowing())
this.focus();else
this._focusOnShow=true;await this._initScope(this._scopesSelector.selected());if(searchImmediately)
this._onAction();else
this._startIndexing();}
async _initScope(scopeTypeId){let extension=scopeTypeId?this._searchScopes.get(scopeTypeId):null;if(!extension)
extension=this._defaultScope;this._searchScope=await extension.instance();}
...
_load(){const searchConfig=Search.SearchConfig.fromPlainObject(this._advancedSearchConfig.get());this._search.value=searchConfig.query();this._matchCaseButton.setToggled(searchConfig.ignoreCase());this._regexButton.setToggled(searchConfig.isRegex());this._selectScope(searchConfig.scopeType());if(this._search.value&&this._search.value.length)
...
_selectScope(scopeTypeId){this._scopesSelector.select(scopeTypeId||this._defaultScope.descriptor().id);}};Search.SearchView.ActionDelegate=class{handleAction(context,actionId){this._showSearch();return true;}
_showSearch(){const selection=UI.inspectorView.element.window().getSelection();let queryCandidate='';if(selection.rangeCount)
queryCandidate=selection.toString().replace(/\r?\n.*/,'');return Search.SearchView.openSearch(null,queryCandidate);}};;Search.SearchConfig=class{constructor(scopeType,query,ignoreCase,isRegex){this._scopeType=scopeType;this._query=query;this._ignoreCase=ignoreCase;this._isRegex=isRegex;this._parse();}
...
regexes.push(createSearchRegex(queries[i],!this._searchConfig.ignoreCase(),this._searchConfig.isRegex()));for(let i=fromIndex;i<toIndex;++i){const lineContent=searchResult.matchLineContent(i);let matchRanges=[];for(let j=0;j<regexes.length;++j)
matchRanges=matchRanges.concat(this._regexMatchRanges(lineContent,regexes[j]));const anchor=Components.Linkifier.linkifyRevealable(searchResult.matchRevealable(i),'');const lineNumberSpan=createElement('span');lineNumberSpan.classList.add('search-match-line-number');lineNumberSpan.textContent=this._labelString(searchResult,i);anchor.appendChild(lineNumberSpan);const contentSpan=this._createContentSpan(lineContent,matchRanges);anchor.appendChild(contentSpan);const searchMatchElement=new UI.TreeElement();searchMatchElement.selectable=false;this.appendChild(searchMatchElement);searchMatchElement.listItemElement.className='search-match source-code';searchMatchElement.listItemElement.appendChild(anchor);}}
_labelString(searchResult,index){const MIN_WIDTH=4;let label=searchResult.matchLabel(index);if(label===null)
return spacesPadding(MIN_WIDTH);label=label.toString();return label.length<MIN_WIDTH?spacesPadding(MIN_WIDTH-label.length)+label:label;}
