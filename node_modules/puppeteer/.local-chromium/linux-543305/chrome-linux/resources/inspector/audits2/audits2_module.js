...
this._reportContainer=this._resultsView.createChild('div','lh-vars lh-root lh-devtools');const dom=new DOM((this._resultsView.ownerDocument));const detailsRenderer=new Audits2.DetailsRenderer(dom);const categoryRenderer=new CategoryRenderer(dom,detailsRenderer);const renderer=new Audits2.Audits2Panel.ReportRenderer(dom,categoryRenderer);const templatesHTML=Runtime.cachedResources['audits2/lighthouse/templates.html'];const templatesDOM=new DOMParser().parseFromString(templatesHTML,'text/html');if(!templatesDOM)
